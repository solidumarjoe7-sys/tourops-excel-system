...
export default function DashboardStats({ 
  ...
  const totalArrivalBookings = arrivals.length;
  const totalArrivalPax = arrivals.reduce((sum, item) => sum + (item.pax || 0), 0);

  // 2. Calculate Departures summary
  const totalDepartureBookings = departures.length;
  const totalDeparturePax = departures.reduce((sum, item) => sum + (item.pax || 0), 0);

  // 3. Count employees with any logs registered
  const activePersonnel = attendance.length;
  const totalLogsRegistered = attendance.reduce((sum, emp) => {
    return sum + Object.keys(emp.logs || {}).length;
  }, 0);
  ...
  // Filter records that have actual collectible values
      ...
      <div id="dashboard-stats-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <button
          ...
          className={`w-full text-left p-4 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-between transition-all active:translate-x-[2px] active:translate-y-[2px] active:shadow-none cursor-pointer rounded-none outline-none ${
            showCollectiblesPanel ? "bg-amber-100 text-amber-950 ring-2 ring-black" : "bg-amber-50 text-slate-930 hover:bg-amber-100"
          }`}
