...
export default function DepartureTable({
  ...
  const handleCellClick = (id: string, field: keyof DepartureRecord, value: any) => {
                ...
                <table className="w-full text-left border-collapse border border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  ...
                  <tbody className="divide-y divide-black font-sans text-xs">
                    {groupRecords.map((row) => (
                      <tr
                        key={row.id}
                        className="hover:bg-amber-50/20 group transition-colors odd:bg-white even:bg-neutral-50"
                      >
                        {/* DATE CELL */}
                        <td className="p-2 border-r border-black align-middle font-mono font-bold">
                          {editingCell?.id === row.id && editingCell?.field === "date" ? (
                            ...
                            <div
                              ...
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer font-mono font-bold text-[#1A1A1A] overflow-hidden text-ellipsis whitespace-nowrap uppercase"
                        ...
                        <td className="p-2 border-r border-black align-middle font-bold">
                          {editingCell?.id === row.id && editingCell?.field === "name" ? (
                            ...
                            <div
                              ...
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer font-bold text-black uppercase overflow-hidden text-ellipsis whitespace-nowrap"
                        ...
                        <td className="p-2 border-r border-black text-center align-middle">
                          {editingCell?.id === row.id && editingCell?.field === "pax" ? (
                            ...
                            <div
                              onClick={() => handleCellClick(row.id, "pax", row.pax)}
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer text-black font-black font-mono text-center"
                              title={t.clickToEditPax}
                            >
                              {row.pax}
                            </div>
                          )}
                        ...
                        {/* AVAIL CELL */}
                        <td className="p-2 border-r border-black align-middle font-bold text-blue-900">
                          {editingCell?.id === row.id && editingCell?.field === "avail" ? (
                            ...
                            <div
                              onClick={() => handleCellClick(row.id, "avail", row.avail)}
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer text-blue-800 font-bold uppercase"
                              title={t.clickToEditService}
                            >
                        ...
                        <td className="p-2 border-r border-black align-middle font-mono font-bold text-black">
                          {editingCell?.id === row.id && editingCell?.field === "number" ? (
                            ...
                            <div
                              onClick={() => handleCellClick(row.id, "number", row.number)}
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer font-mono font-bold text-slate-800"
                        ...
                        {/* HOTEL RECOVERY */}
                        <td className="p-2 border-r border-black align-middle font-bold text-[#1A1A1A]">
                          {editingCell?.id === row.id && editingCell?.field === "hotel" ? (
                            ...
                            <div
                              onClick={() => handleCellClick(row.id, "hotel", row.hotel)}
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer font-bold text-black uppercase"
                              title={t.clickToEditRecovery}
                            >
                        ...
                        {/* ETD CELL */}
                        <td className="p-2 border-r border-black align-middle font-black text-amber-900 font-mono">
                          {editingCell?.id === row.id && editingCell?.field === "etd" ? (
                            ...
                            <div
                              onClick={() => handleCellClick(row.id, "etd", row.etd)}
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer font-black text-slate-700 font-mono uppercase"
                              title={t.clickToEditETD}
                            >
                        ...
                        {/* PICKUP TIME CELL */}
                        <td className="p-2 border-r border-black align-middle font-black text-blue-900 font-mono">
                          {editingCell?.id === row.id && editingCell?.field === "pickupTime" ? (
                            <input
                              ...
                              onChange={(e) => setTempValue(e.target.value)}
                              className="w-full p-1 text-xs border border-black rounded-none focus:outline-hidden bg-white text-blue-700 font-mono font-bold uppercase"
                            />
                          ) : (
                            <div
                              onClick={() => handleCellClick(row.id, "pickupTime", row.pickupTime)}
                              className="py-1 px-1.5 hover:bg-amber-100 transition-colors cursor-pointer font-black text-blue-800 font-mono uppercase"
                              title={t.clickToEditPickup}
                            >
