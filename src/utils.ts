...
export function exportArrivalsToCSV(arrivals: ArrivalRecord[]) {
  let csv = "DATE,NAME,PAX,AVAILABILITY/TRANSFER,CONTACT NUMBER,HOTEL,ETA,COLLECT/PAYMENT\r\n";
  ...
  sorted.forEach((row) => {
    ...
    csv += `${row.date},${escapedName},${row.pax},${escapedAvail},${escapedNumber},${escapedHotel},${escapedEta},${escapedCollect}\r\n`;
  });
...
export function exportDeparturesToCSV(departures: DepartureRecord[]) {
  let csv = "DATE,NAME,PAX,AVAILABILITY/TRANSFER,CONTACT NUMBER,HOTEL,ETD,PICK UP TIME\r\n";
  ...
  sorted.forEach((row) => {
    ...
    csv += `${row.date},${escapedName},${row.pax},${escapedAvail},${escapedNumber},${escapedHotel},${escapedEtd},${escapedPickup}\r\n`;
  });
